...
class FqnFactory:
    ...
    def create_identity_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.domain.identities.{SnakeString(aggregate_name)}_id")

    @staticmethod
    def create_repository_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.domain.repositories.{SnakeString(aggregate_name)}_repository")
    ...
    def create_sql_alchemy_unit_of_work_fqn(context: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.infrastructure.persistence.repositories.sql_alchemy_unit_of_work")

    @staticmethod
    def create_dto_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.application.dtos.{SnakeString(aggregate_name)}_dto")
    ...
    @staticmethod
    def create_update_entity_from_dto_mapper_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        snake_name = SnakeString(aggregate_name)
    ...
    def create_create_command_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.application.commands.create_{SnakeString(aggregate_name)}")

    @staticmethod
    def create_update_command_fqn(context: str, aggregate_name: str) -> ModuleFqn:
        return ModuleFqn(f"{context}.application.commands.update_{SnakeString(aggregate_name)}")

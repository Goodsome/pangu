from code_dom.domain.value_objects.ast_stmt import AstClassDef, AstStmtBase
...
def class_def_to_ast_class_def(class_def: ClassDef) -> AstClassDef:
    ...
    body: list[AstStmtBase] = [method_def_to_ast_function_def(m) for m in class_def.methods]
    return AstClassDef(
        name=class_def.name,
        bases=bases,
        keywords=[],

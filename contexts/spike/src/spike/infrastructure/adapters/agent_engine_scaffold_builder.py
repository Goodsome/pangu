...
from spike.domain.exceptions import InternalError
...
class AgentEngineScaffoldBuilder(ScaffoldBuilder):
    ...
    async def build(
        ...
        args = [
            ...
            system_prompt_file,
            "--context-payload",
            scaffold_payload.model_dump_json(),
        ]
        try:
            ...
            logger.error(
                f"subprocess.CalledProcessError: {e}\n  stdout: {str(e.stdout)}\n  stderr: {str(e.stderr)}"
            )
            raise InternalError("Failed to build scaffold", cause=e) from e

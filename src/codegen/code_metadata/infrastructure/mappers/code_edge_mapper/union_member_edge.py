...
class UnionMemberEdgeMapper(BaseEdgeMapper):

    @staticmethod
    def to_dto(edge_model: CodeEdgeModel, direction: EdgeDirection) -> UnionMemberEdge:
        return UnionMemberEdge(
            fqn=BaseEdgeMapper._get_target_fqn(edge_model, direction),
            direction=direction,
        )

    @staticmethod
    def to_properties(dto: UnionMemberEdge) -> dict[str, Any]:
        return {}

...
class ReferencesEdgeMapper(BaseEdgeMapper):

    @staticmethod
    def to_dto(edge_model: CodeEdgeModel, direction: EdgeDirection) -> ReferencesEdge:
        return ReferencesEdge(
            fqn=BaseEdgeMapper._get_target_fqn(edge_model, direction),
            direction=direction,
        )

    @staticmethod
    def to_properties(dto: ReferencesEdge) -> dict[str, Any]:
        return {}
